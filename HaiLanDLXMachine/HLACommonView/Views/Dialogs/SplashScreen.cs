...
namespace HLACommonView.Views.Dialogs
{
    public sealed class SplashScreen : Control
    {
        ...
        private string m_strSystemTitleString;
        ...
        {
            SystemTitleString = string.Empty;
            TitleString = string.Empty;
            CommentaryString = string.Empty;
            BackgroundImage = Resource.splashbg;
            Font = new System.Drawing.Font("微软雅黑", 14F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if (Bounds.Width > 0 && Bounds.Height > 0 && Visible)
            {
                try
                {
                    ...
                    m_nTextOffsetY = Convert.ToInt32(Math.Ceiling(((nClientHeight / 3) * 2))) - 30;
                    /*系统名称*/
                    if (SystemTitleString != string.Empty)
                    {
                        Font fontSystemTitle = new Font("宋体", 30, FontStyle.Bold);
                        SizeF sizeF = g.MeasureString(SystemTitleString, fontSystemTitle, ClientRectangle.Width, m_stringFormat);
                        m_nTextOffsetY += Convert.ToInt32(Math.Ceiling(sizeF.Height));
                        RectangleF rectangleF = new RectangleF(ClientRectangle.Left + 5, ClientRectangle.Top + 85, sizeF.Width, sizeF.Height);

                        SolidBrush brushFont = new SolidBrush(Color.DarkGreen);
                        g.DrawString(SystemTitleString, fontSystemTitle, brushFont, rectangleF, m_stringFormat);
                        brushFont.Dispose();
                        fontSystemTitle.Dispose();
        ...
        #region Comments
        /// <summary>
        /// Sets the system title text displayed by the splash screen
        /// </summary>
        #endregion
        public static void SetSystemTitleString(string title)
        {
            Instance.SystemTitleString = title;
        }
        #region Comments
        /// <summary>
        ...
        private string SystemTitleString
        {
            get
            {
                return m_strSystemTitleString;
            }
            set
            {
                m_strSystemTitleString = value;
                Refresh();
            }
        }
