...
{
    public partial class InventoryMainForm : CommonPMInventoryForm
    {
        ...
        List<CTagDetail> getTags(ResultDataInfo record)
        {
        ...
        void addGrid(ResultDataInfo record, SapResult uploadResult)
        {
            try
            {
                if (uploadResult != null && !uploadResult.SUCCESS)
                {
                    List<CTagDetail> tags = getTags(record);
                    if (tags != null && tags.Count > 0)
                    {
                        Invoke(new Action(() =>
                        {
                            foreach (var v in tags)
                            {
                                if (IsYupinxiang())
                                {
                                    ListViewItem error = new ListViewItem(record.BoxNO);
                                    error.SubItems.Add("");
                                    error.SubItems.Add(v.zsatnr);
                                    error.SubItems.Add(v.zcolsn);
                                    error.SubItems.Add(v.zsiztx);
                                    error.SubItems.Add(v.quan.ToString());
                                    error.SubItems.Add(uploadResult.MSG);
        ...
        void clearData(bool reset = false)
        {
            //清除当前屏幕统计数量
            errorEpcNumber = 0;
            mainEpcNumber = 0;
            ...
            Invoke(new Action(() =>
            {
                lblWorkStatus.Text = reset ? "停止" : "正在盘点";
        ...
        private void EnqueueErrorRecord()
        {
            if (currentErrorRecordList.Count > 0)
            {
                foreach (ErrorRecord record in currentErrorRecordList)
                {
                    ...
                    */
                    LocalDataService.SaveErrorRecord(record, mReceiveType);
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            if(string.IsNullOrEmpty(lblBoxNo.Text.Trim()))
            {
                MessageBox.Show("请先扫描箱号", "提示",MessageBoxButtons.OK, MessageBoxIcon.Warning);
                lblBoxNo.Focus();
                return;
            }
...
#if DEBUG
            /*
            reportEpc("50000027A0000B0000001");
            reportEpc("50000027A0000B0000002");
            reportEpc("50000027A0000B0000003");
            reportEpc("50000027A0000B0000004");
            reportEpc("50000027A0000B0000005");
            reportEpc("50000027A0000B0000006");
            reportEpc("50000027A0000B0000007");
            reportEpc("50000027A0000B0000008");
            reportEpc("50000027A0000B0000009");
            reportEpc("50000027A0000B000000A");
            reportEpc("50000027A0000B000000B");
            reportEpc("50000027A0000B000000C");
            reportEpc("50000027A0000B000000D");
            reportEpc("50000027A0000B000000E");
            reportEpc("50000027A0000B000000F");
            */
            /*
            reportEpc("50000027B0000B0000001");
            reportEpc("50000027B0000B0000002");
            reportEpc("50000027B0000B0000003");
            reportEpc("50000027B0000B0000004");

            reportEpc("50000027C0000B0000001");
            reportEpc("50000027C0000B0000002");

            reportEpc("50000027D0000B0000001");
            */
#endif
        }
